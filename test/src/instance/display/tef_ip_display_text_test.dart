...
import '../../../../testing/mocks/models/display_text_request_model_mock.dart';
...
void main() {
  group('TefIPDisplayText', () {
    late TefIPDisplayText displayText;
    late MockHttpClient kHttpClient;

    setUp(() {
      kHttpClient = MockHttpClient();
      displayText = TefIPDisplayText();
    });
    ...
    test('should return SuccessResponseModel on success', () async {
      ...
      final result = await displayText.post(
        displayTextRequest: kDisplayTextRequest,
        client: kHttpClient,
      );
      ...
      verify(
        () => kHttpClient.post(
          ...
          headers: any(named: 'headers'),
          body: jsonEncode(kDisplayTextRequest),
        ),
      ).called(1);
    });
