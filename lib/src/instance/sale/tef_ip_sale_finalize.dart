...
/// Endpoint responsible for finalizing the current sale.
///
/// Performs an HTTP `POST` request to `/sale/finalize`
...
@immutable
@protected
interface class TefIPSaleFinalize implements EndpointInterface {
  /// Fixed endpoint path.
  @override
  String get endpoint => TefIPEndpoints.saleFinalize;

  /// Finalizes the current sale.
  ///
  /// - [params] optional display parameters shown after finalization.
  Future<SuccessResponseModel> post({
    SaleActionRequestModel? params,
    http.Client? client,
    Duration? timeout,
  }) async {
    try {
      return await TefIPNetworkingClient.post<SuccessResponseModel>(
        ...
        body: params != null ? jsonEncode(params.toJson()) : null,
        client: client,
        timeout: timeout,
