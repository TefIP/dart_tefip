...
@immutable
@protected
interface class TefIPAskForm implements EndpointInterface {
  /// Fixed endpoint path.
  @override
  ...
  Future<List<AnswerModel>> post({
    ...
    try {
      return await TefIPNetworkingClient.post<List<AnswerModel>>(
        url: TefIpUrlBuilder.build(endpoint),
        body: jsonEncode(form.toJson()),
        client: client,
        onSuccess: (json) => List<AnswerModel>.from(
          json.map((e) => AnswerModel.fromJson(e)),
        ),
      );
